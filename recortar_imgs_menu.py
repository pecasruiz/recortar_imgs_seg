...
import recortar_imgs as core  # requiere que definas core.main(in_dir,out_dir,ref_path,pad,preview,scale_view)

def run_main(in_dir, out_dir, ref_path, pad, preview, scale_view, btn):
    try:
        btn.config(state="disabled")
        core.main(in_dir, out_dir, ref_path if ref_path else None, pad, preview, scale_view)
...
def start():
    ...
    btn_run = btn_start
    t = threading.Thread(target=run_main, args=(in_dir, out_dir, ref_path, pad, preview_var.get()==1, scale, btn_run), daemon=True)
    t.start()
